...
fs.readFile(basePath, function (err, data) {
    if (err) {
        ...
        let theme = process.env.npm_config_primary ? process.env.npm_config_primary : '409EFF'
        let variable = '$--color-primary:#' + theme + ';'
                            ...
                            if (err) {
                                ...
                                if (stream) {
                                    ...
                                    stream.on('end', function () {
                                        ...
                                        //写入本地某个路径
                                        //用户自定义目标路径
                                        let base = '../../../../src/assets/css/index_' + theme + '.css'
                                        let destPath = path.resolve(__dirname, base)
                                        console.log(destPath)
                                        fs.writeFile(destPath, finalData, "binary", function (err) {
                                            if (err) {
                                                console.log("下载失败");
                                            } else
                                                console.log("样式文件生成成功");
                                            //在utils/theme中引入

                                            let export_path = path.resolve(__dirname, '../../../../src/utils/theme.js')
                                            fs.readFile(export_path, function (err, data) {
                                                if (err) {
                                                    ...
                                                    let str = decoder.write(Buffer.from(data)) //str为string类型文件 
                                                    if (str.indexOf(theme) > -1) {
                                                        console.log('该样式已存在')
                                                    } else {
                                                        let theme_append = str.slice(-30)
                                                        let theme_data = str.replace('\}', '').replace('export default themeMap', '').trim()
                                                        let theme_variable = 'const index_' + theme + ' = () => import(\'@/assets/css/index_' + theme + '.css\')'
                                                        let thememap_variable = '\'#' + theme + '\': index_' + theme + ','
                                                        fs.writeFile(export_path, theme_variable + '\r\n' + theme_data + '\r\n' + '    ' + thememap_variable + theme_append, function (err) {
                                                            if (err)
                                                                console.log('写入主题色导出失败')
                                                            else
                                                                console.log('主题色导出文件写入成功')
                                                        })
                                                    }
                                                }
                                            })
