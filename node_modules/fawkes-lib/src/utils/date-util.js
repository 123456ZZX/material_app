import fecha from 'fawkes-lib/src/utils/date'
import { t } from 'fawkes-lib/src/locale'
...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`fks.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => t(`fks.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month => t(`fks.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => t(`fks.datepicker.month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}
