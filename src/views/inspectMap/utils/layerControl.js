...
const defaultSource = {
  院内矢量服务: {
    ...
    // tiles: [
    //   // API.VUE_APP_MAP_TILE_URL + 
    //   // "http://172.23.0.123:8008/maps/osm/{z}/{x}/{y}.pbf"
    //   // "http://mapservergcy.hdec.com/maps/osm/{z}/{x}/{y}.pbf"
    //   // "http://118.195.235.225:8088/maps/osm/{z}/{x}/{y}.pbf"
    //   // 本地
    //   // API.VUE_APP_MAP_YN_URL + "/swan-base-map/mapserver/pbfServer/mvt/619d79e6-62da-11ec-bd00-005056c00008/mapbox.mapbox-streets-v8/{z}/{x}/{y}.pbf"
    //   // 打包
...
export default {
  async addLayers() {
    let map = window.rxMap;
    axios.get('http://172.20.79.65/api/swan/mapWorkshopSource/detail?id=1544640278559821826').then(res => {
      ...
      layers.layers.forEach(layer => {
        ...
        map.addLayer(layer);
        map.on("click", layer.id, this.handleMonitorClick);
        map.on("mouseenter", layer.id, this.handleMonitorEnter);
        map.on("mouseout", layer.id, this.handleMonitorLeave);
      })
    }).catch(e => { }).finally(() => {
      // let borderSource= {
      //   type: "raster",
      //   tiles: [
      //     "http://172.20.79.121:8080/iserver/services/map-RongDongFanWei4326_20220718/wmts100/%E5%AE%B9%E4%B8%9C%E8%BE%B9%E7%95%8C/default/GoogleMapsCompatible_容东边界/{z}/{y}/{x}.png"
      //   ],
      //   tileSize: 256
      // }
    });
    ...
    // await request({
    //   url: "/smartwork-package/constructionSite/portal/child",
    //   method: "get"
    // }).then(res => {
    //   let dataList = []
    ...
    //       geometry: {
    //         type: "Point",
    //         coordinates: item.constructionPost && item.constructionPost.split(",").length == 2 ? 
    //           item.constructionPost.split(",") : [0, 0]
    //       },
    //       properties: {
    //         ...item,
    //         type: item.constructionType ? item.constructionType : "隧洞工地"
    //       }
    //     });
    //   });
    //   map.addSource("gongdiSource", {
    //     "type": "geojson",
    //     data: {
