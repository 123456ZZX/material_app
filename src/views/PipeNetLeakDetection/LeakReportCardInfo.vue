<!-- '漏点上报信息卡片' -->
<template>
  <div class="problemCardInfo">
    <div class="leftImgStyle">
      <div class="imgStyle">
        <img :src="cardInfo.formValue.imgs[0].url || ''" alt="" />
      </div>
    </div>
    <div class="rightTextStyleRight">
      <div class="commonStyle topTitle">
        <!-- 标题 -->
        <div class="topTitleIn" @click="toDetailPanel(cardInfo)">
          <span class="text">{{ cardInfo.formValue.title }}</span>
        </div>
        <!-- 右侧定位图标 -->
        <img
          @click="flyToPoint(cardInfo.formValue.ticketAddress)"
          src="@/assets/img/inspection/定位.png"
          alt=""
        />
      </div>
      <!-- 设备编码 -->
      <div class="commonStyle centerText">
        <span class="text">{{ cardInfo.formValue.deviceCode }}</span>
      </div>
      <!-- 现场描述 -->
      <div class="commonStyle bottomText">
        <span class="text">{{ cardInfo.formValue.questionDescribe }}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 定位到某一点
    flyToPoint(lnglat) {
      console.log(this.cardInfo.formValue)
      console.log(typeof lnglat, lnglat)
      let ll = null
      if (typeof lnglat == 'string') {
        ll = JSON.parse(JSON.parse(lnglat).point)
        console.log(ll)
      } else {
        ll = lnglat.point
        console.log(ll)
      }
      this.$emit('flyToPoint', [parseFloat(ll.lng), parseFloat(ll.lat)])
    },
    // 进入详情页
