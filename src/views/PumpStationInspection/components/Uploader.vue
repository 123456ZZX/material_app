    ...
    >
      ...
      <template #input>
        ...
        >
          ...
          <slot name="upload-area" v-if="type==='image'">
            <fm-button type="primary" icon="add-o" plain @click.stop="onClickUploads" style="z-index: 99;"/>
            <!-- <div @click.stop="onClickUploads" style="z-index: 99;"> 
              <i class="fm-icon fm-icon-add-o" size="middle"></i>
            </div> -->
          </slot>
...
export default {
  ...
  methods: {
    ...
    uploadFile(file) {
      ...
      {
        isWaterMark:1,
        waterMark:this.$moment().format('YYYY-MM-DD HH:mm:ss'),
        fontSize:140,
        waterMarkLocation:1
      }
      //文件读取完成后的回调函数
      ...
      formData.append('waterMarkJson',JSON.stringify(markers))
    ...
    takePhoto(){
      yuanchu.imagePicker.takePhoto(this.onSuccess, this.onFail,  { 
            width : 1920, 
            height : 1440, 
            quality : 50 
        });
