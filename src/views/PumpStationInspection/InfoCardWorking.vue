...
<template>
  <div class="InfoCard" @click="execute(cardInfo)">
    ...
    <div class="topSide">
      ...
      <div class="bottomTitle">{{ cardInfo.patrolRouteName }}</div>
    </div>
    <!-- 巡检信息 -->
    <div class="bottomSide">
      <div class="commonStyle restTime">
        <img src="../../assets/img/inspection/剩余时间.png" alt="" />
        <div class="titleText">任务计划开始时间:</div>
        <span class="taskStartTime">{{ this.cardInfo.taskStartTime | format  }}</span>
      ...
      <div class="commonStyle keyPoint">
        ...
        <div class="bigNumber">{{ cardInfo.finishPatrolPointNum }}</div>
        <div class="smallText">/</div>
        <div class="smallText">{{ cardInfo.patrolPointNum }}</div>
...
export default {
  ...
  computed: {
    // 右上角图片地址
    imageUrl() {
      return require(`../../assets/img/inspection/厂站.png`)
    },
    // 倒计时
    timeRest() {
      return new Date(this.cardInfo.taskStartTime)
    },
  ...
  filters: {
    format(content) {
        return  moment(content).format('MM-DD HH:mm')
    }
  },  
  data() {
    return {
