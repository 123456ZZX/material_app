 ...
 * @LastEditTime: 2022-07-22 10:21:58
 * @Description: 封装axios
 */
...
request.interceptors.request.use(
  config => {
    ...
    if(config.sign){
      ...
      verifyUrl(config, 'sign') ? config.params = getSign(config.params) : config.headers['Fawkes-Auth'] = `${storage.get('access_token')}`
    }
    apiCrypto.requestEncrypt.encrypt && !apiCrypto.requestEncrypt.ignoreUrl && (config = encrypt(config))
    return config
  },
  error => {
      ...
      switch (error.response.status) {
        ...
        case 401:
          switch (error.response.data.code) {
            case -8000160: {
              confirmHandler('无证书提示', error.response.data.message, action => {
                router.push({ path: '/activate', query: { keepOrigin: true } })
              })
            } break
            case -8000150: {
